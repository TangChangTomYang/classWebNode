...
## 2. letter-spacing / word-spacing 属性
...
伪类选择器的作用是给已经存在的类选择器增加一些功能, 比如:

1. 可以通过`动态伪类`为已经存在的选择器增加一种`动态选择`的功能
2. 
...
- `:nth-child(num)` 
  ...
  选择span元素, 并且span元素是他父元素中的第一个子元素, **如果父元素的第一个元素不是span就不选** 

```
/*选择span元素, 并且span元素是他父元素中的第一个子元素*/
span:nth-child(1){
	color:red;
}
```
